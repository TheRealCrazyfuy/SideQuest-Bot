...
client.once(Events.ClientReady, (readyClient) => {
  ...
  logStandardMessage(
    `${readyClient.user.tag} restarted and is now online.`,
    readyClient,
  );
});
  ...
  if (interaction.isChatInputCommand()) {
    ...
    try {
      ...
      if (interaction.replied || interaction.deferred) {
        ...
        logErrorMessage(
          `Error executing command ${interaction.commandName} for user ${interaction.user.id}: ${error}`,
          interaction.client,
        );
    ...
    for (const roleId of roleIds) {
      try {
        ...
        logErrorMessage(
          `Failed to add role ${roleId} to user ${member.id}: ${err}`,
          interaction.client,
        );
      }
    }
    ...
    interaction.customId.startsWith("forum_help_")
  ) {
    ...
    try {
      ...
      await interaction.reply({
        content: "Sorry the initial thread message got deleted.",
        flags: MessageFlags.Ephemeral,
      });
      return;
    }
    ...
    if (topic) {
      logStandardMessage(
        `Provided help topic "${topic.name}" to user <@${interaction.user.id}> in thread "${interaction.channel.url}".`,
        interaction.client,
      );
      await interaction.reply({
        content: topic.reply,
    ...
    if (interaction.channel.isThread()) {
      try {
        ...
        logErrorMessage(
          `Error sending closing confirmation on ${interaction.channel.url}: ${err}`,
          interaction.client,
        );
        await interaction.reply({
    ...
    interaction.isButton() &&
    interaction.customId === "confirm_close_thread"
  ) {
    try {
      if (
        interaction.user.id == interaction.channel.ownerId ||
        ...
        await interaction.reply({
          content:
            "Thread has been closed successfully <:Mora_Agree:1380160309771374624>.",
        });
      ...
      logErrorMessage(
        `Error closing thread ${interaction.channel.url}: ${err}`,
        interaction.client,
      );
...
client.on("threadCreate", async (thread) => {
  ...
  if (
    ...
    try {
      ...
      let afterReply =
        "When your issue is resolved, please remember to close the thread by clicking the 'Close Thread' button below or doing `/close` command, thank you <:mora_cheer:925660965448609842>.\n\n";
      ...
      if (possibleHelpTopics.length > 0) {
        ...
        await thread.send({
          content: preReply + afterReply,
          components: [row],
        });
      }
    } catch (err) {
      logErrorMessage(
        `Error replying to thread ${thread.url}: ${err}`,
        thread.client,
      );
      console.error(
        "Error fetching starter message or replying to a thread:",
        err,
      );
    }
  }
